 ...
 */
@Configuration
@ConditionalOnProperty(prefix = "erp.muti-datasource", name = "open", havingValue = "true")
...
//    }
    ...
    @Bean
    @ConfigurationProperties(prefix = "erp.muti-datasource")
    public MutiDataSourceProperties mutiDataSourcePropertiesErp() {
        return new MutiDataSourceProperties();
    }
     ...
     * 多数据源配置
     */
    private DruidDataSource erpDataSource(DruidProperties druidProperties, MutiDataSourceProperties mutiDataSourcePropertiesErp) {
        DruidDataSource dataSource = new DruidDataSource();
        druidProperties.config(dataSource);
        mutiDataSourcePropertiesErp.config(dataSource);
        return dataSource;
    }
     ...
     */
    @Bean
    public DynamicDataSource mutiDataSource(DruidProperties druidProperties, MutiDataSourceProperties mutiDataSourcePropertiesErp) {

        DruidDataSource dataSource = dataSource(druidProperties);
        DruidDataSource erpDataSource = erpDataSource(druidProperties, mutiDataSourcePropertiesErp);
        try {
            dataSource.init();
            erpDataSource.init();
        ...
        hashMap.put(mutiDataSourcePropertiesErp.getDataSourceNames()[0], dataSource);
        hashMap.put(mutiDataSourcePropertiesErp.getDataSourceNames()[1], erpDataSource);
